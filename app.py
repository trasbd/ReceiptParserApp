...
def index():
    if request.method == 'POST':
        ...
        if file:
            ...
            category = matched_receipt.category if matched_receipt and matched_receipt.category else None

            new_receipt = Receipt(
                store=result['store'],
                date=result['date'],
                total=float(result['total']) if result['total'] != "Total not found" else 0.0,
...
def admin():
    ...
    if filter_option == 'day':
        ...
        query = query.filter(Receipt.date.between(start.strftime('%Y-%m-%d'), end.strftime('%Y-%m-%d')))
    elif filter_option == 'month':
        query = query.filter(Receipt.date.startswith(today.strftime('%Y-%m')))
    elif filter_option == 'range' and start_date and end_date:
        query = query.filter(Receipt.date.between(start_date.strftime('%Y-%m-%d'), end_date.strftime('%Y-%m-%d')))

    receipts = query.all()
    total = sum(r.total for r in receipts)

    return render_template('admin.html', receipts=receipts, headers=["Store", "Date", "Total", "Category", "Image"],
                           total=f"{total:.2f}", filter=filter_option,
